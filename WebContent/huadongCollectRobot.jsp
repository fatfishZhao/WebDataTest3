...
<style>
  ...
  height:35px;
   border:2px solid #ccc;
	...
	border-radius: 20px;
	background: -moz-linear-gradient(top, #7D9EC0	, 	#7D9EC0	);
	background: -webkit-gradient(linear, left top, left bottom, color-stop(0, #7D9EC0	),
		color-stop(1, 	#7D9EC0	));
...
#Submit{ 
  ...
  $(function() {
	  ...
	  $( "#draggable1" ).draggable({axis:"x", containment: "#containment-wrapper", scroll: false ,
	      drag: function(event, ui) {
	      },
	      stop: function(event, ui) {
	        var y=event.offsetY;
	        if (ui.position.left > 205&&ui.position.left<232) {$(this).fadeOut(100,function(){
				flag++;
				if(flag==3){
				var sub = document.getElementById("Submit");
				sub.style.visibility="visible";
				}
	        });}else{$(this).animate({left: 0});}
	      }});
	  
	  $( "#draggable2" ).draggable({axis:"x", containment: "#containment-wrapper", scroll: false ,
	      drag: function(event, ui) {
	      },
	      stop: function(event, ui) {
	        var y=event.offsetY;
	        if (ui.position.left > 305&&ui.position.left<332) {$(this).fadeOut(100,function(){
	        	flag++;
				if(flag==3){
				var sub = document.getElementById("Submit");
				sub.style.visibility="visible";
	  ...
	  $( "#draggable3" ).draggable({axis:"x", containment: "#containment-wrapper", scroll: false ,
	      ...
	      stop: function(event, ui) {
	        var y=event.offsetY;
	        if (ui.position.left > 405&&ui.position.left<432) {$(this).fadeOut(100,function(){
	        	flag++;
				if(flag==3){
				var sub = document.getElementById("Submit");
				sub.style.visibility="visible";
...
<center><p style="font-family: '微软雅黑';-webkit-text-stroke: 1px #E5E5E5;font-weight:bold;font-size:60px;color:#6495ED">横向滑动解锁数据采集</p></center>

<div class="containment-wrapper" id="wrapper1" style="position:absolute; top:35%;left:10%;">

  <div id="draggable1" class="draggable" style="position: relative;"> </div>
  <canvas id="myCanvas1" width="49" height="33" style="border:1px solid #3d3d3d;position:absolute;top:10px;left:230px;">
...
<form id="form1" name="form1"  method="post" action="/WebDataTest3/MainServlet">
  <input type="hidden" name="method" value="huadongCollectRobot" />
  <input type="hidden" name="StrData" id="StrData"> 
  <input type="hidden"   name="StrData2" id="StrData2">
  <input type="hidden"   name="robotID" id="robotID">  
...
<div class="containment-wrapper" id="wrapper2" style="position:absolute; top:50%;left:10%;">

  <div id="draggable2" class="draggable" style="position: relative;"> </div>
  <canvas id="myCanvas2" width="49" height="33" style="border:1px solid #3d3d3d;position:absolute;top:10px;left:330px;"></canvas> </div>

<div class="containment-wrapper" id="wrapper3" style="position:absolute; top:65%;left:10%;">

  <div id="draggable3" class="draggable" style="position: relative;"> </div>
  <canvas id="myCanvas3" width="49" height="33" style="border:1px solid #3d3d3d;position:absolute;top:10px;left:430px;">
...
<script>
  ...
  function browerInfo(){
	  var x=navigator;
	  document.getElementById("robotID").value = "<%= request.getParameter("robotID")%>"
...
//鼠标移动数据获取
  function mousemove_sendXML(){
	 ...
	 StringData=StringData+"HDHorizontal"+" "+"mousemove"+" "+x+" "+y+" "+d+"\n";
	 document.getElementById("StrData").value = StringData;
	 
     var clientX=e.clientX;
     var clientY=e.clientY;	 
	   strCilent=strCilent+"HDHorizontal"+" "+"mousemove"+" "+clientX+" "+clientY+" "+d+"\n";
  ...
  function mousedown_sendXML(){
      ...
      StringData=StringData+"HDHorizontal"+" "+"mousedown"+" "+x+" "+y+" "+d+"\n";
      document.getElementById("StrData").value = StringData;

      var clientX=e.clientX;
      var clientY=e.clientY;	 
	   strCilent=strCilent+"HDHorizontal"+" "+"mousedown"+" "+clientX+" "+clientY+" "+d+"\n";
	   document.getElementById("StrData2").value = strCilent;	
  ...
  var mouseDrag = function(event){
		...
		var x = e.screenX;
		var y = e.screenY;
		StringData = StringData + "HDHorizontal" + " " + "mousedrag" + " " + x + " " + y
				+ " " + d + "\n";
		document.getElementById("StrData").value = StringData;
		
	      var clientX=e.clientX;
	      var clientY=e.clientY;	 
		   strCilent=strCilent+"HDHorizontal"+" "+"mousedrag"+" "+clientX+" "+clientY+" "+d+"\n";
		   document.getElementById("StrData2").value = strCilent;
  ...
  //up
    function mouseup_sendXML(){
	  ...
	  var x = e.screenX ;
      var y = e.screenY ;
      StringData=StringData+"HDHorizontal"+" "+"mouseup"+" "+x+" "+y+" "+d+"\n"; 
      document.getElementById("StrData").value = StringData;
      
      var clientX=e.clientX;
      var clientY=e.clientY;	 
	   strCilent=strCilent+"HDHorizontal"+" "+"mouseup"+" "+clientX+" "+clientY+" "+d+"\n";
	   document.getElementById("StrData2").value = strCilent;	
  ...
  var getCoordInDocumentExample = function(){
	...
	//击键数据获取
	 function keyDown(){
          ...
          var d = (new Date()).valueOf();   		
          if(e.keyCode!=229){
             	StringData=StringData+"HDHorizonal"+" "+"keydown"+" "+e.keyCode+" "+d+"\n"; 
              strCilent=strCilent+"HDHorizonal"+" "+"keydown"+" "+e.keyCode+" "+d+"\n";        	 
           }else{
          	 StringData=StringData+"illegal "+"HDHorizonal"+" "+"keydown"+" "+e.keyCode+" "+d+"\n"; 
               strCilent=strCilent+"illegal "+"HDHorizonal"+" "+"keydown"+" "+e.keyCode+" "+d+"\n";     	 
           }
      		document.getElementById("StrData").value = StringData;
        	 document.getElementById("StrData2").value = strCilent;
               }
	 ...
	 function keyUp(){
         ...
         var d = (new Date()).valueOf();
          StringData=StringData+"HDHorizonal"+" "+"keyUp"+" "+e.keyCode+" "+d+"\n";
          document.getElementById("StrData").value = StringData;
          
          strCilent=strCilent+"HDHorizonal"+" "+"keyup"+" "+e.keyCode+" "+d+"\n";
          document.getElementById("StrData2").value = strCilent; 
